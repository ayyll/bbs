		...
		<div class="row">
			<div class="col-md-8">
				...
				<c:forEach var="t" items="${articlePageBean.list }">
					<c:if test="${t.status eq 1 or t.status eq 3}">
						<div class="page-header">
							<h4>
								<span class="label label-primary" onclick="window.location.href='${pageContext.request.contextPath}/article/top'" style="cursor:pointer">置顶</span>
								<c:if test="${t.status eq 3}">
									<span class="label label-danger" onclick="window.location.href='${pageContext.request.contextPath}/article/perfect'" style="cursor:pointer">精</span>
								</c:if>
				...
				<c:choose>
					...
					<c:otherwise>
						<center>
							<h3><a href="javascript:void(0);" data-toggle="modal" data-target="#myModal">登陆</a>后才可进行发帖！</h3>
						</center>
					</c:otherwise>
				</c:choose>
					...
					<center>
						<c:choose>
							...
							<c:otherwise>
								...
								<br><br>用户名：
								${user.username }<br>
								等级${user.rank }---<span style="color:red">${user.head_title }</span>
