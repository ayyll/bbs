...
<body>
	<div class="container" style="box-shadow: 0px 0px 1px #666;">
		<a href="${pageContext.request.contextPath}/index.jsp"> <img
			...
			class="img-responsive" alt="安徽大学"> </a><br>
		<c:choose>
			<c:when test="${user.uid eq u.uid }">
				<div class="row">
					<div class="col-md-2">
						<div class="info">
							<img alt="headimg" src="<c:url value="${u.headimg }"/>"
								class="img-thumbnail">
							<center>
								<h3>ID:${u.uid }</h3>
							</center>
					...
					<div class="col-md-8">
						<h3>
							...
							<button class="btn btn-primary" type="btn" onclick="updateUser()"
								style="float: right;">修改密码</button>
						</h3>
						<div class="form-group">
							<form
								action="${pageContext.request.contextPath}/user/headimg/${u.uid }"
								enctype="multipart/form-data" method="post">
								<input type="file" id="imgUpload" name="file" accept="image/jpg,image/jpeg,image/png,image/gif"><br>
								...
								<c:forEach var="ua" items="${userArticle }">
									<tr>
										<td><span class="glyphicon glyphicon-tag"
											...
											href="${pageContext.request.contextPath}/article/details/${ua.aid }"
											target="_blank"> <c:out value="${ua.title }"></c:out> </a>
											<div style="float: right;">
												<fmt:formatDate value="${ua.date }"
													pattern="yyyy/MM/dd HH:mm:ss" />
												<button type="button" class="btn btn-danger"
													onclick="deleteArticle(${ua.aid })">删除</button>
											</div></td>
									</tr>
								</c:forEach>
							</table>
